...
namespace Rivet
{
  class HERWIG_JETS_CH_JP_UE : public Analysis
  {
  public:
    RIVET_DEFAULT_ANALYSIS_CTOR(HERWIG_JETS_CH_JP_UE);
    ...
    void init()
    {
	 ...
	 abort();
      ...
      mytxtfile.open("herwig_20_pthat_20_pt_jets_ch_jp_ue_matched_ue.txt");
      mytxtfile_long.open("herwig_20_pthat_20_pt_jets_ch_jp_ue.txt");
	...
	jets_s[i].set_user_info(new fastjet::MyUserInfo(q, -1, -999));
     ...
     // Match jets to partons (don't do this for herwig)
  ...
  RIVET_DECLARE_PLUGIN(HERWIG_JETS_CH_JP_UE);
}
