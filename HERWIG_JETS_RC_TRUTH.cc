...
namespace Rivet
{
  class HERWIG_JETS_RC_TRUTH : public Analysis
  {
  public:
    RIVET_DEFAULT_ANALYSIS_CTOR(HERWIG_JETS_RC_TRUTH);
    ...
    void init()
    {
      ...
      declare(fs, "fs");
      // open output file
      mytxtfile.open("herwig_nodecay_PTMINpthatPTMAX_rc_truth.txt");
    }
    ...
    void analyze(const Event &event)
    {
      ...
      for (HepMC::GenEvent::particle_iterator p = theEvent->particles_begin(); p != theEvent->particles_end(); ++p)
      {
        ...
        for (unsigned int i = 0; i < jets.size(); i++)
        {
          ...
          // parton_pseudojet.set_user_index(i); // record that the ith jet is matched to the parton
          break;
        }
      }
      ...
      for (unsigned int i = 0; i < jets.size(); i++)
      {
        ...
        for (unsigned int j = 0; j < IncParts.size(); j++)
        {
          ...
          if (part.user_info<fastjet::ParticleInfo>().parents().size() == 0)
          {
            cout << "no parent" << endl;
            continue;
          }
        ...
        else if (parents1.size() > 1) // if more than one parent, then save the non partonic one
        {
          for (const Particle &parent : parents1)
          {
            if (abs(parent.pid()) < 10 || abs(parent.pid()) == 21)
            {
              continue;
            }
            parent1 = parent.pid();
        ...
        else if (parents2.size() > 1) // if more than one parent, then save the non partonic one
        {
          for (const Particle &parent : parents2)
          {
            if (abs(parent.pid()) < 10 || abs(parent.pid()) == 21)
            {
              continue;
        ...
        mytxtfile << evid << ", " << xsecweight << ", " << jets[i].perp() << ", " << jets[i].constituents().size() << ", " << jets[i].user_index() << ", " << ChargedParts.size() << ", " << ChargedParts.at(0).perp() << ", " << ChargedParts.at(0).user_info<fastjet::ParticleInfo>().charge() << ", " << ChargedParts.at(0).eta() << ", " << ChargedParts.at(0).phi() << ", " << ChargedParts.at(0).user_info<fastjet::ParticleInfo>().pid() << ", " << parent1 << ", " << ChargedParts.at(1).perp() << ", " << ChargedParts.at(1).user_info<fastjet::ParticleInfo>().charge() << ", " << ChargedParts.at(1).eta() << ", " << ChargedParts.at(1).phi() << ", " << ChargedParts.at(1).user_info<fastjet::ParticleInfo>().pid() << ", " << parent2 << ", " << dr << ", " << m << ", " << parents1.size() << ", " << parents2.size() << "\n";
      }
    }
  ...
  RIVET_DECLARE_PLUGIN(HERWIG_JETS_RC_TRUTH);
}