...
namespace Rivet
{
  class HERWIG_JETS_DIJET : public Analysis
  {
  public:
    RIVET_DEFAULT_ANALYSIS_CTOR(HERWIG_JETS_DIJET);
    ...
    void init()
    {
      ...
      // open output file
      eecfile.open("herwig_PTMINpthatPTMAX_eec.txt");
    }

    double angle(fastjet::PseudoJet &a, fastjet::PseudoJet &b)
    {
	double ax = a.px();
    	double ay = a.py();
    	double az = a.pz();
  ...
  RIVET_DECLARE_PLUGIN(HERWIG_JETS_DIJET);
}
