...
namespace Rivet
{
  class HERWIG_JETS_EEC : public Analysis
  {
  public:
    RIVET_DEFAULT_ANALYSIS_CTOR(HERWIG_JETS_EEC);
    ...
    void init()
    {
      ...
      // open output file
      trkfnfile.open("herwig_PTMINpthatPTMAX_trkfn_kp.txt");
      eecfile.open("herwig_PTMINpthatPTMAX_eec_kp.txt");
    }
    ...
    void analyze(const Event &event)
    {
      ...
      for (unsigned int i = 0; i < jets.size(); i++)
      {
        ...
        trkfnfile << evid << ", " << i << ", " << xsecweight << ", " << theEvent->event_scale() << ", " << jets[i].perp() << ", " << jets[i].eta() << ", " << nkplus << ", " << nkminus << ", " << npiplus << ", " << npiminus << ", " << xkplus << ", " << xkminus << ", " << xpiplus << ", " << xpiminus << endl;
        ...
        for (unsigned int k = 0; k < jets[i].constituents().size(); k++)
        {
          ...
          {
            //cout << abs(kaon_candidate.user_info<fastjet::ParticleInfo>().pid()) << endl;
	    continue;
          }
	    ...
	    PseudoJet assoc = jets[i].constituents().at(p);
            ...
            {
              //cout << abs(assoc.user_info<fastjet::ParticleInfo>().pid()) << endl;
	      continue;
            }
            ...
            eecfile << evid << ", " << i << ", " << jets[i].perp() << ", " << pid_sum << ", " << weight << ", " << dr << endl;
    ...
    RIVET_DECLARE_PLUGIN(HERWIG_JETS_EEC);
  }
