...
namespace Rivet
{
  class PYTHIA_JETS_0810_WBADTOWERS_TRUTHWODECAY_WTOWERGAP : public Analysis
  {
  public:
    RIVET_DEFAULT_ANALYSIS_CTOR(PYTHIA_JETS_0810_WBADTOWERS_TRUTHWODECAY_WTOWERGAP);
    ...
    void analyze(const Event &event)
    {
      ...
      for (unsigned int i = 0; i < fsParticles.size(); i++)
      {
        ...
        if (p.fromHadron() && (parent.abspid() == 111 || parent.abspid() == 211 || parent.abspid() == 221 || parent.abspid() == 321 || parent.abspid() == 310 || parent.abspid() == 130 || parent.abspid() == 3122 || parent.abspid() == 3212 || parent.abspid() == 3112 || parent.pid() == 3222 || parent.abspid() == 3312 || parent.abspid() == 3322 || parent.abspid() == 3334)) // from a hadron weak decay
        {
          ...
          if (parent.abspid() != 111 && parent.abspid() != 211 && parent.abspid() != 221 && parent.abspid() != 321 && parent.abspid() != 310 && parent.abspid() != 130 && parent.abspid() != 3122 && parent.abspid() != 3212 && parent.abspid() != 3112 && parent.pid() != 3222 && parent.abspid() != 3312 && parent.abspid() != 3322 && parent.abspid() != 3334)
          {
            cout << "unexpected parent: " << parent.pid() << endl;
          }
      ...
      for (const Particle &p : fsParticles)
      {
        ...
        if (neutral)
        {
          ...
          if (pj_smeared.Et() < 0.2 || pj_smeared.Et() > 2000. || abs(pj_smeared.eta()) > 1.0)
            continue;
          ...
          for (int j = 0; j < 20; j++)
          {
            if ((abs(phi_smeared - phi_gap_list[j])) < 0.006283)
            // neutral particle falls in phi gap
            {
              bad_tower = true;
              break;
            }
        ...
        else
        {
          ...
          // THIS IS NOT THE DEFAULT
          if (pj_smeared.perp() < 0.2 || pj_smeared.perp() > 2000. || abs(pj_smeared.eta()) > 1.0)
            continue;
          pj_smeared.set_user_info(new fastjet::ParticleInfo(charge, pid));
          parts_s.push_back(pj_smeared);
        }
  ...
  private:
    ...
    double phi_gap_list[20] = {-3.12902628, -2.81486702, -2.50070775, -2.18654849, -1.87238922, -1.55822996, -1.24407069, -0.92991143, -0.61575216, -0.30159289, 0.01256637,  0.32672564,  0.6408849 ,  0.95504417,  1.26920343, 1.5833627 ,  1.89752196,  2.21168123,  2.52584049,  2.83999976};
  ...
  RIVET_DECLARE_PLUGIN(PYTHIA_JETS_0810_WBADTOWERS_TRUTHWODECAY_WTOWERGAP);
}