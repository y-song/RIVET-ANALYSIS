...
namespace Rivet
{
  class PYTHIA_JETS_UNDOTRUTHDECAY : public Analysis
  {
  public:
    RIVET_DEFAULT_ANALYSIS_CTOR(PYTHIA_JETS_UNDOTRUTHDECAY);
    ...
    void init()
    {
      ...
      // open output file
      mytxtfile.open("pythia_20pthat25_test.txt");
    ...
    void analyze(const Event &event)
    {
      ...
      // test undoing geant decays
      /*PseudoJets parts_s_test;
      int isJP2_test = 0;
      double JP2_et_test = -1;
      int JP2_eta_test = -1;
      int JP2_phi_test = -1;
      e_JP_test.clear();
      for (int i = 0; i <= Nbounds_JP_eta; ++i)
      {
        ...
        e_JP_test.push_back(xgrid);
        xgrid.clear();
      }
      //-------------------------------------------
      // fill geant-level particles (also undo decay)
      for (const Particle &p : pythiaParticles)
      {
        if (p.abspid() == 12 || p.abspid() == 14 || p.abspid() == 16)
          continue;
        ...
        if (neutral)
        {
          ...
          if (pj_smeared.Et() < 0.2 || abs(pj_smeared.eta()) > 1.0)
            continue;
          pj_smeared.set_user_info(new fastjet::ParticleInfo(charge, pid));
          parts_s_test.push_back(pj_smeared);
        }
        else
        {
          uniform_real_distribution<double> dropCharge(0.0, 1.0);
      ...
      //===========================================
      // check if event passes JP2 trigger
      for (PseudoJet &pj_smeared : parts_s_test)
      {
        for (int x = 0; x < Nbounds_JP_eta; ++x)
        {
          for (int y = 0; y < Nbounds_JP_phi; ++y)
      ...
      for (int x = 0; x < Nbounds_JP_eta; ++x)
      {
        ...
        for (int y = 0; y < Nbounds_JP_phi; ++y)
        {
          if (e_JP_test[x][y] > 7.3)
          {
            isJP2_test = 1;
            JP2_et_test = e_JP_test[x][y];
            JP2_eta_test = x;
            JP2_phi_test = y;
            break;
          }
      ...
      /*if (isJP2 == 0 && isJP2_test == 1)
      {
        cout << "\nundecayed event passed JP " << evid;
        cout << "\nundecayed: ";
        for (int x = 0; x < Nbounds_JP_eta; ++x)
        {
          for (int y = 0; y < Nbounds_JP_phi; ++y)
      ...
      if (isJP2 == 1 && isJP2_test == 0)
        cout << "\ndecayed event passed JP " << evid << endl;*/
  ...
  private:
    ...
    vector<vector<double>> e_JP_test;
  };

  RIVET_DECLARE_PLUGIN(PYTHIA_JETS_UNDOTRUTHDECAY);
}