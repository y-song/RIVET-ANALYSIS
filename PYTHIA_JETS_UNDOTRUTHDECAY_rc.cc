...
namespace fastjet
{
  ...
  {
  public:
    DihadronInfo(const double &b_ch, const double &b, const double &y1_ch, const double &y2_ch, const double &phi1_ch, const double &phi2_ch, const int &pid1_ch, const int &pid2_ch, const double &y1, const double &y2, const double &phi1, const double &phi2, const int &pid1, const int &pid2, const double &q, const int &parton) : _b_ch(b_ch), _b(b), _y1_ch(y1_ch), _y2_ch(y2_ch), _phi1_ch(phi1_ch), _phi2_ch(phi2_ch), _pid1_ch(pid1_ch), _pid2_ch(pid2_ch), _y1(y1), _y2(y2), _phi1(phi1), _phi2(phi2), _pid1(pid1), _pid2(pid2), _q(q), _parton(parton) {}
...
namespace Rivet
{
  class PYTHIA_JETS_UNDOTRUTHDECAY_rc : public Analysis
  {
  public:
    RIVET_DEFAULT_ANALYSIS_CTOR(PYTHIA_JETS_UNDOTRUTHDECAY_rc);
    ...
    void init()
    {
      ...
      // open output file
      mytxtfile.open("pythia_50pthat_rc_wdecay.txt");
    ...
    void analyze(const Event &event)
    {
      ...
      for (unsigned int i = 0; i < jets.size(); i++)
      {
        ...
        else
        {
          ...
          pid1 = (IncPart.at(0)).user_info<fastjet::ParticleInfo>().pid();
          pid2 = (IncPart.at(1)).user_info<fastjet::ParticleInfo>().pid();
        }
        double numerator = 0;
        ...
        else
        {
          ...
          pid1_ch = (ChargedPart.at(0)).user_info<fastjet::ParticleInfo>().pid();
          pid2_ch = (ChargedPart.at(1)).user_info<fastjet::ParticleInfo>().pid();
        }
        jets[i].set_user_index(-1);
        jets[i].set_user_info(new fastjet::DihadronInfo(b_ch, b, y1_ch, y2_ch, phi1_ch, phi2_ch, pid1_ch, pid2_ch, y1, y2, phi1, phi2, pid1, pid2, q, -999));
        IncPart.clear();
        ChargedPart.clear();
      }
      ...
      for (unsigned int is = 0; is < jets_s.size(); is++)
      {
        ...
        else
        {
          ...
          pid1s = (IncPart.at(0)).user_info<fastjet::ParticleInfo>().pid();
          pid2s = (IncPart.at(1)).user_info<fastjet::ParticleInfo>().pid();
        }
        double numerator = 0;
        for (unsigned int j = 0; j < IncPart.size(); j++)
        {
        ...
        else
        {
          ...
          pid1s_ch = (ChargedPart.at(0)).user_info<fastjet::ParticleInfo>().pid();
          pid2s_ch = (ChargedPart.at(1)).user_info<fastjet::ParticleInfo>().pid();
        }
        jets_s[is].set_user_index(-1);
        jets_s[is].set_user_info(new fastjet::DihadronInfo(bs_ch, bs, y1s_ch, y2s_ch, phi1s_ch, phi2s_ch, pid1s_ch, pid2s_ch, y1s, y2s, phi1s, phi2s, pid1s, pid2s, qs, -999));
        IncPart.clear();
        ChargedPart.clear();
      }
      ...
      for (unsigned int is = 0; is < jets_s.size(); is++)
      {
        ...
        mytxtfile << "-9, -9, -9, -9, -9, -9, -9, -9, -9, -9, -9, -9, -9, -9, " << jets_s[is].user_info<fastjet::DihadronInfo>().b_ch() << ", " << jets_s[is].user_info<fastjet::DihadronInfo>().b() << ", " << jets_s[is].user_info<fastjet::DihadronInfo>().y1_ch() << ", " << jets_s[is].user_info<fastjet::DihadronInfo>().y2_ch() << ", " << jets_s[is].user_info<fastjet::DihadronInfo>().phi1_ch() << ", " << jets_s[is].user_info<fastjet::DihadronInfo>().phi2_ch() << ", " << jets_s[is].user_info<fastjet::DihadronInfo>().pid1_ch() << ", " << jets_s[is].user_info<fastjet::DihadronInfo>().pid2_ch() << ", " << jets_s[is].user_info<fastjet::DihadronInfo>().y1() << ", " << jets_s[is].user_info<fastjet::DihadronInfo>().y2() << ", " << jets_s[is].user_info<fastjet::DihadronInfo>().phi1() << ", " << jets_s[is].user_info<fastjet::DihadronInfo>().phi2() << ", " << jets_s[is].user_info<fastjet::DihadronInfo>().pid1() << ", " << jets_s[is].user_info<fastjet::DihadronInfo>().pid2() << ", -9, " << jets_s[is].user_info<fastjet::DihadronInfo>().q() << ", ";
  ...
  RIVET_DECLARE_PLUGIN(PYTHIA_JETS_UNDOTRUTHDECAY_rc);
}