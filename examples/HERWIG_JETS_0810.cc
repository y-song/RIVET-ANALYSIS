...
namespace Rivet
{
  class HERWIG_JETS_0810 : public Analysis
  {
  public:
    RIVET_DEFAULT_ANALYSIS_CTOR(HERWIG_JETS_0810);
    ...
    void init()
    {
      ...
      // open output file
      mytxtfile.open("herwig_50pthat.txt");
	...
	for (vector<double>::iterator it = jet_q.begin(); it != jet_q.end(); ++it)
        ...
        mytxtfile << xsecweight << ", " << missing << ", " << jets[i].perp() << ", " << jets[i].m() << ", " << jets[i].constituents().size() << ", " << parton << ", " << sdjets[i].m() << ", " << sdjets[i].structure_of<fastjet::contrib::SoftDrop>().delta_R() << ", " << sdjets[i].structure_of<fastjet::contrib::SoftDrop>().symmetry() << ", " << sdjets[i].constituents().size() << ", " << -999 << ", " << -999 << ", " << -999 << ", " << -999 << ", " << -999 << ", " << -999 << ", " << -999 << ", " << -999 << ", " << i << ", " << theEvent->event_scale() << "\n";
      }
      for (unsigned int is = 0; is < jets_s.size(); is++)
      {
        if (jets_s[is].user_info<fastjet::MyUserInfo>().matching() == matched)
 	...
 	mytxtfile << "-999, ";
        ...
        mytxtfile << xsecweight << ", " << fake << ", -999, -999, -999, -999, -999, -999, -999, -999, " << jets_s[is].perp() << ", " << jets_s[is].m() << ", " << jets_s[is].constituents().size() << ", " << parton_s << ", " << sdjets_s[is].m() << ", " << sdjets_s[is].structure_of<fastjet::contrib::SoftDrop>().delta_R() << ", " << sdjets_s[is].structure_of<fastjet::contrib::SoftDrop>().symmetry() << ", " << sdjets_s[is].constituents().size() << ", -999, -999\n";
      }
    }
  ...
  RIVET_DECLARE_PLUGIN(HERWIG_JETS_0810);
}
