...
namespace Rivet
{
  class PYTHIA_JETS_FOR_XT : public Analysis
  {
  public:
    RIVET_DEFAULT_ANALYSIS_CTOR(PYTHIA_JETS_FOR_XT);
    ...
    void init()
    {
	 ...
	 abort();
      ...
      mytxtfile.open("pythia_10_pthat_10_pt_jets_full_matched_no_hadronization.txt");
      mytxtfile_long.open("pythia_10_pthat_10_pt_jets_full_no_hadronization.txt");
	...
	pseudojet.set_user_info(new fastjet::ParticleInfo(charge,pid));
      ...
      // jet selectors
      fastjet::Selector selector = fastjet::SelectorPtMin(10.0)*fastjet::SelectorEtaRange(-0.5,0.5);
	...
	jets_s[i].set_user_info(new fastjet::MyUserInfo(q, -1, -999));
      ...
      for (HepMC::GenEvent::particle_iterator p = theEvent -> particles_begin(); p!= theEvent -> particles_end(); ++p) {
      	...
      	if ( (*p) -> status() == 21 ) { // incoming hard scattered partons
		if ( abs((*p) -> pdg_id()) <= 6 ) {
			process *= 2;
		}
		else if ( (*p) -> pdg_id() == 21 ) {
	...
	for (unsigned int i = 0; i < jets.size(); i++){
		if (jets[i].delta_R(parton_pseudojet) > 0.5 || jets[i].user_info<fastjet::MyUserInfo>().parton() != -999)	continue;
	...
	for (unsigned int is = 0; is < jets_s.size(); is++){
		if (jets_s[is].delta_R(parton_pseudojet) > 0.5 || jets_s[is].user_info<fastjet::MyUserInfo>().parton() != -999)	continue;
      ...
      if (process != 4 && process != 6 && process != 9)
      ...
      for (unsigned int i = 0; i < jets.size(); i++)
      {
        for (unsigned int is = 0; is < jets_s.size(); is++)
        {
          if (jets_s[is].user_info<fastjet::MyUserInfo>().matching() == matched)
            continue;
          if (jets[i].delta_R(jets_s[is]) < 0.5)
	...
	mytxtfile_long << xsecweight << ", " << fake << ", -999, -999, -999, -999, -999, -999, -999, -999, -999, -999, " << jets_s[is].perp() << ", " << jets_s[is].m() << ", " << jets_s[is].constituents().size() << ", " << jets_s[is].eta() << ", " << jets_s[is].phi() << ", " << parton_s << ", " << mgs << ", " << rgs << ", " << zgs << ", -999, " << is << ", -999, " << trans_s_pt.size() << ", -999, " << trans_s_05_pt.size() << ", -999, " << trans_s_ptavg << ", -999, " << trans_s_05_ptavg << ", -999, -999, -999, " << isJP2 <<  ", " << JP2_et << ", " << JP2_eta << ", " << JP2_phi << "\n";
      }
  ...
  RIVET_DECLARE_PLUGIN(PYTHIA_JETS_FOR_XT);
}
