...
namespace Rivet
{
  class HERWIG_JETS_rc_random : public Analysis
  {
  public:
    RIVET_DEFAULT_ANALYSIS_CTOR(HERWIG_JETS_rc_random);
    ...
    void init()
    {
      ...
      // open output file
      mytxtfile.open("herwig_45pthat50_rc_random.txt");
	...
	int n_mult = IncPart.size();
	vector<int> vec_full(n_mult);
        iota(vec_full.begin(), vec_full.end(), 0);
        random_device rd;
        mt19937 g(rd());
        shuffle(vec_full.begin(), vec_full.end(), g);   
        //vector<PseudoJet> IncPartVec = fastjet::sorted_by_pt(IncPart.constituents());
        ...
        {
	  int leading_index = vec_full.at(0);
	  int subleading_index = vec_full.at(1);
          b = (IncPart.at(leading_index)).user_info<fastjet::ParticleInfo>().charge() * (IncPart.at(subleading_index)).user_info<fastjet::ParticleInfo>().charge();
          y1 = (IncPart.at(leading_index)).rap();
          y2 = (IncPart.at(subleading_index)).rap();
          phi1 = (IncPart.at(leading_index)).phi();
          phi2 = (IncPart.at(subleading_index)).phi();
        }
       double numerator = 0;
	...
	vector<int> vec(nch);
        ...
        {
          int leading_index = vec.at(0);
	  int subleading_index = vec.at(1);
	  b_ch = (ChargedPart.at(leading_index)).user_info<fastjet::ParticleInfo>().charge() * (ChargedPart.at(subleading_index)).user_info<fastjet::ParticleInfo>().charge();
          y1_ch = (ChargedPart.at(leading_index)).rap();
          y2_ch = (ChargedPart.at(subleading_index)).rap();
          phi1_ch = (ChargedPart.at(leading_index)).phi();
          phi2_ch = (ChargedPart.at(subleading_index)).phi();
        }
...
RIVET_DECLARE_PLUGIN(HERWIG_JETS_rc_random);
}
