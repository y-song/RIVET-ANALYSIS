...
namespace Rivet
{
	...
	public:
		...
		void init()
		{
			...
			mytxtfile.open("pythia_test_0923.txt");
			mytxtfile_long.open("pythia_test_full_0923.txt");
		...
		void analyze(const Event &event)
		{
			...
			if (parts_ch.size() != 0 && jets.size() != 0)
			{
				...
				{
					if (abs(jets[0].delta_phi_to(part_ch)) < pi / 3 || abs(jets[0].delta_phi_to(part_ch)) > 2 * pi / 3)
						continue;
					if (part_ch.perp() > 0.2)
					{
			...
			if (parts_s_ch.size() != 0 && jets_s.size() != 0)
			{
				...
				{
					if (abs(jets_s[0].delta_phi_to(part_s_ch)) < pi / 3 || abs(jets_s[0].delta_phi_to(part_s_ch)) > 2 * pi / 3)
						continue;
			...
			if (trans_pt.size() != 0)
				trans_ptavg = accumulate(trans_pt.begin(), trans_pt.end(), 0.0) / trans_pt.size();
			if (trans_s_pt.size() != 0)
				trans_s_ptavg = accumulate(trans_s_pt.begin(), trans_s_pt.end(), 0.0) / trans_s_pt.size();
			if (trans_05_pt.size() != 0)
				trans_05_ptavg = accumulate(trans_05_pt.begin(), trans_05_pt.end(), 0.0) / trans_05_pt.size();
			if (trans_s_05_pt.size() != 0)
				trans_s_05_ptavg = accumulate(trans_s_05_pt.begin(), trans_s_05_pt.end(), 0.0) / trans_s_05_pt.size();

			vector<double> half{};
			vector<double> half_s{};

			// Calculate jet charge and half-moment
			for (unsigned int i = 0; i < jets.size(); i++)
			{
				double half_temp = 0;
				// double m2_test_trk = 0;
				vector<double> q{};
				for (unsigned int j = 0; j < jets[i].constituents().size(); j++)
				{
					PseudoJet part = jets[i].constituents().at(j);
					double dR = jets[i].delta_R(part);
					// cout << "dR = " << dR << endl;
					// m_test_trk +=  part.perp()/jets[i].perp()*dR;
					half_temp += part.perp() / jets[i].perp() * pow(dR, 0.5);
				}
				// m_test.push_back( m_test_trk );
				half.push_back(half_temp);
			...
			// Calculate smeared jet charge and half-moment
			for (unsigned int i = 0; i < jets_s.size(); i++)
			{
				...
				for (unsigned int j = 0; j < jets_s[i].constituents().size(); j++)
				{
					...
					double dR = jets_s[i].delta_R(part);
					half_temp += part.perp() / jets_s[i].perp() * pow(dR, 0.5);
				}
				half_s.push_back(half_temp);
			...
			for (unsigned int i = 0; i < jets.size(); i++)
			{
				for (unsigned int is = 0; is < jets_s.size(); is++)
				{
					...
					if (jets[i].delta_R(jets_s[is]) < 0.4)
					{
						...
						int nch = 0;
						int nch_s = 0;
						for (const PseudoJet &p : jets[i].constituents())
						{
							if (p.user_info<fastjet::ParticleInfo>().charge() != 0)
							{
								nch += 1;
							}
						}
						for (const PseudoJet &p : jets_s[is].constituents())
						{
							if (p.user_info<fastjet::ParticleInfo>().charge() != 0)
							{
								nch_s += 1;
						...
						mytxtfile << xsecweight << ", " << matched << ", " << jets[i].perp() << ", " << jets[i].m() << ", " << half[i] << ", " << jets[i].constituents().size() << ", " << nch << ", " << jets[i].eta() << ", " << jets[i].phi() << ", " << parton << ", " << mg << ", " << rg << ", " << zg << ", " << jets_s[is].perp() << ", " << jets_s[is].m() << ", " << half_s[is] << ", " << jets_s[is].constituents().size() << ", " << nch_s << ", " << jets_s[is].eta() << ", " << jets_s[is].phi() << ", " << parton_s << ", " << mgs << ", " << rgs << ", " << zgs << ", " << i << ", " << is << ", " << trans_pt.size() << ", " << trans_s_pt.size() << ", " << trans_05_pt.size() << ", " << trans_s_05_pt.size() << ", " << trans_ptavg << ", " << trans_s_ptavg << ", " << trans_05_ptavg << ", " << trans_s_05_ptavg << ", " << jets[i].delta_R(partons.at(0)) << ", " << jets[i].delta_R(partons.at(1)) << ", " << theEvent->event_scale() << ", " << isJP2 << ", " << JP2_et << ", " << JP2_eta << ", " << JP2_phi << "\n";
						mytxtfile_long << xsecweight << ", " << matched << ", " << jets[i].perp() << ", " << jets[i].m() << ", " << half[i] << ", " << jets[i].constituents().size() << ", " << nch << ", " << jets[i].eta() << ", " << jets[i].phi() << ", " << parton << ", " << mg << ", " << rg << ", " << zg << ", " << jets_s[is].perp() << ", " << jets_s[is].m() << ", " << half_s[is] << ", " << jets_s[is].constituents().size() << ", " << nch_s << ", " << jets_s[is].eta() << ", " << jets_s[is].phi() << ", " << parton_s << ", " << mgs << ", " << rgs << ", " << zgs << ", " << i << ", " << is << ", " << trans_pt.size() << ", " << trans_s_pt.size() << ", " << trans_05_pt.size() << ", " << trans_s_05_pt.size() << ", " << trans_ptavg << ", " << trans_s_ptavg << ", " << trans_05_ptavg << ", " << trans_s_05_ptavg << ", " << jets[i].delta_R(partons.at(0)) << ", " << jets[i].delta_R(partons.at(1)) << ", " << theEvent->event_scale() << ", " << isJP2 << ", " << JP2_et << ", " << JP2_eta << ", " << JP2_phi << "\n";
						break;
					}
				}
			}
			...
			for (unsigned int i = 0; i < jets.size(); i++)
			{
				...
				mytxtfile_long << "-999, -999, -999, -999, -999, " << xsecweight << ", " << missing << ", " << jets[i].perp() << ", " << jets[i].m() << ", " << half[i] << ", " << jets[i].constituents().size() << ", " << nch << ", " << jets[i].eta() << ", " << jets[i].phi() << ", " << parton << ", " << mg << ", " << rg << ", " << zg << ", -999, -999, -999, -999, -999, -999, -999, -999, -999, -999, -999, " << i << ", -999, " << trans_pt.size() << ", -999, " << trans_05_pt.size() << ", -999, " << trans_ptavg << ", -999, " << trans_05_ptavg << ", -999, " << jets[i].delta_R(partons.at(0)) << ", " << jets[i].delta_R(partons.at(1)) << ", " << theEvent->event_scale() << ", " << isJP2 << ", " << JP2_et << ", " << JP2_eta << ", " << JP2_phi << "\n";
			}
			...
			for (unsigned int is = 0; is < jets_s.size(); is++)
			{
				...
				mytxtfile_long << xsecweight << ", " << fake << ", -999, -999, -999, -999, -999, -999, -999, -999, -999, -999, -999, " << jets_s[is].perp() << ", " << jets_s[is].m() << ", " << half_s[is] << ", " << jets_s[is].constituents().size() << ", " << nch_s << ", " << jets_s[is].eta() << ", " << jets_s[is].phi() << ", " << parton_s << ", " << mgs << ", " << rgs << ", " << zgs << ", -999, " << is << ", -999, " << trans_s_pt.size() << ", -999, " << trans_s_05_pt.size() << ", -999, " << trans_s_ptavg << ", -999, " << trans_s_05_ptavg << ", -999, -999, -999, " << isJP2 << ", " << JP2_et << ", " << JP2_eta << ", " << JP2_phi << "\n";
			}
		}
