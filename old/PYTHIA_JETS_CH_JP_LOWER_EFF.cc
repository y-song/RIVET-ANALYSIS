...
namespace Rivet
{
  class PYTHIA_JETS_CH_JP_LOWER_EFF : public Analysis
  {
  public:
    RIVET_DEFAULT_ANALYSIS_CTOR(PYTHIA_JETS_CH_JP_LOWER_EFF);
    ...
    void init()
    {
	 ...
	 abort();
      ...
      mytxtfile.open("pythia_20_pthat_jets_ch_jp_lower_eff_matched.txt");
      //mytxtfile_long.open("pythia_15_pthat_jets_ch_jp_test1222.txt");
	...
	pseudojet.set_user_info(new fastjet::ParticleInfo(charge,pid));
      ...
      fastjet::ClusterSequence cs(parts_ch, jet_def);
      ...
      fastjet::ClusterSequence cs_s(parts_s_ch, jet_def);
      vector<fastjet::PseudoJet> jets_s = sorted_by_pt(selector(cs_s.inclusive_jets()));
      vector<fastjet::PseudoJet> sdjets_s = sd(jets_s);

      /*if (!jets_s.empty()) {
	     cout << "smeared leading charged jet pT = " << jets_s[0].perp() << endl;
      	     if (isJP2 == 0){
      		for (int x = 0; x < Nbounds_JP_eta; ++x) {
	      		for (int y = 0; y < Nbounds_JP_phi; ++y) {
		      		cout << "JP bins (eta,phi) =  (" << x << ", " << y << "), eT = " << e_JP[x][y] << endl;
	      		}
		}
      	     }
      }*/
       	...
       	if (sdjets[i] == 0) // did not pass the softdrop requirement
	{
        ...
        mytxtfile_long << -999 << ", " << -999 << ", " << -999 << ", " << -999 << ", " << -999 << ", " << -999 << ", " << -999 << ", " << -999 << ", " << xsecweight << ", " << missing << ", " << jets[i].perp() << ", " << jets[i].m() << ", " << jets[i].constituents().size() << ", " << jets[i].eta() << ", " << jets[i].phi() << ", " << parton << ", " << mg << ", " << rg << ", " << zg << ", " << -999 << ", " << -999 << ", " << -999 << ", " << -999 << ", " << -999 << ", " << -999 << ", " << -999 << ", " << -999 << ", " << i << ", " << jets[i].delta_R(partons.at(0)) <<", " << jets[i].delta_R(partons.at(1)) << ", " << theEvent->event_scale() << ", " << isJP2 <<  ", " << JP2_et << ", " << JP2_eta << ", " << JP2_phi << "\n";}
	...
	mytxtfile_long << xsecweight << ", " << fake << ", -999, -999, -999, -999, -999, -999, -999, -999, " << jets_s[is].perp() << ", " << jets_s[is].m() << ", " << jets_s[is].constituents().size() << ", " << jets_s[is].eta() << ", " << jets_s[is].phi() << ", " << parton_s << ", " << mgs << ", " << rgs << ", " << zgs << ", -999, -999, -999, -999, " << isJP2 <<  ", " << JP2_et << ", " << JP2_eta << ", " << JP2_phi << "\n";
      }*/
  ...
  RIVET_DECLARE_PLUGIN(PYTHIA_JETS_CH_JP_LOWER_EFF);
}
