...
namespace Rivet
{
  class HERWIG_JETS_1026 : public Analysis
  {
  public:
    RIVET_DEFAULT_ANALYSIS_CTOR(HERWIG_JETS_1026);
    ...
    void init()
    {
      ...
      // open output file
      mytxtfile.open("herwig_25pthat30_trkfn.txt");
	   ...
	   for (unsigned int j = 0; j < jets[i].constituents().size(); j++)
           {
	    	...
	    	if ( part.user_info<fastjet::ParticleInfo>().pid() == 2212 )
	    	{	
			xp += part.perp()/jets[i].perp();
   		}
		else if ( part.user_info<fastjet::ParticleInfo>().pid() == -2212 )
	    	{	
			xpbar += part.perp()/jets[i].perp();
   		}
	   }
	   mytxtfile << evid << ", " << i << ", " << xsecweight << ", " << theEvent->event_scale() << ", " << jets[i].perp() << ", " << jets[i].eta() << ", " << xp << ", " << xpbar << endl;		
       }   
    }
  ...
  RIVET_DECLARE_PLUGIN(HERWIG_JETS_1026);
}
