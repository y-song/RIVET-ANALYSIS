...
namespace Rivet
{
  class PYTHIA_JETS_0810_WBADTOWERS : public Analysis
  {
  public:
    RIVET_DEFAULT_ANALYSIS_CTOR(PYTHIA_JETS_0810_WBADTOWERS);

    void init()
    {
      // THIS IS NOT THE DEFAULT!
      const FinalState fs(Cuts::pt > 0.2 && Cuts::pt < 2000 && Cuts::abseta < 1.0);
      declare(fs, "fs");
      ...
      // open output file
      mytxtfile.open("pythia_wbadtowers_wdecay_test3.txt");
    ...
    void analyze(const Event &event)
    {
      ...
      for (const Particle &p : fsParticles)
      {
        ...
        if (neutral)
        {
          ...
          for (int j = 0; j < 235; j++)
          {
	    // note that the phi ranges from pj and for the list are different! need to fix!!!!!
            if ((abs(pj_smeared.eta()) - bad_eta_list[j]) < 0.025 && abs(pj_smeared.phi() - bad_phi_list[j]) < 0.025)
      ...
      // jet selectors
      // THIS IS NOT THE DEFAULT!
      fastjet::Selector selector = fastjet::SelectorPtMin(0.2) * fastjet::SelectorEtaRange(-0.6, 0.6);
  ...
  RIVET_DECLARE_PLUGIN(PYTHIA_JETS_0810_WBADTOWERS);
}
